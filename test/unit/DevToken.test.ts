...
developmentChains.includes(network.name)
  ? describe("DevToken", async function () {
      async function deployTokenFixture() {
        ...
        console.log(`OWNER IS ${await tokenFactory.owner()}`);
      ...
      describe("Constructor", async function () {
        it("sets the name of the dev tokens correctly", async function () {
          const { devToken1, devToken2 } = await loadFixture(
            deployTokenFixture
          );
      ...
      describe("Burn", async function () {
        it("should not allow users to call the operatorBurn function", async function () {
          const { devToken1, devToken2, deployer } = await loadFixture(
            deployTokenFixture
          );
        ...
        it("should not allow unauthorized users to call the devBurn function", async function () {
          const { devToken1, devToken2, tester } = await loadFixture(
            deployTokenFixture
          );
          const amount = ethers.utils.parseEther("1");
          const bytes = new Uint8Array([0x01, 0x02, 0x03, 0x04]);
          ...
          await expect(
            devToken1.connect(tester).devBurn(tester.address, amount)
          ).to.be.revertedWithCustomError(
            devToken1,
            "DevToken__NotTokenFactory"
          );
          await expect(
            devToken2.connect(tester).devBurn(tester.address, amount)
          ).to.be.revertedWithCustomError(
            devToken2,
            "DevToken__NotTokenFactory"
          );
      ...
      describe("Mint", async function () {
        it("should not allow other users to perform mint function except Token Factory", async function () {
          const { devToken1, deployer, tester } = await loadFixture(
            deployTokenFixture
          );
          const amount = ethers.utils.parseEther("1");
          await expect(
