...
developmentChains.includes(network.name) ?
  describe("TokenFactory", async function () {
    async function deployTokenFixture() {
      ...
      const MockERC20TokenWithoutPermit = await ethers.getContractFactory('MockERC20Token', deployer)
      const underlyingTokenWithoutPermit = await MockERC20TokenWithoutPermit.deploy();
      await underlyingTokenWithoutPermit.deployed();

      const TokenFactory3 = await ethers.getContractFactory('TokenFactory', deployer)
      const tokenFactory3 = await TokenFactory3.deploy(underlyingTokenWithoutPermit.address, mockV3Aggregator.address, REBASE_INTERVAL, sanctionsContract.address);
      await tokenFactory3.deployed();

      // deploy devtoken 1  for the token factory without permit  
      const DevTokenX = await ethers.getContractFactory("DevToken", deployer);
      const devTokenX = await DevTokenX.deploy(TOKEN1_NAME, TOKEN1_SYMBOL, tokenFactory3.address, defaultOperators, sanctionsContract.address);
      await devTokenX.deployed();

      // deploy devtoken 2  for the token factory without permit  
      const DevTokenY = await ethers.getContractFactory("DevToken", deployer);
      const devTokenY = await DevTokenY.deploy(TOKEN2_NAME, TOKEN2_SYMBOL, tokenFactory3.address, defaultOperators, sanctionsContract.address);
    ...
    describe("Deposit", async function () {
      ...
      describe("Deposit with underlying asset that does not support permit", async function () {
        ...
        it("it should revert when user wants to deposit more than maximum amount", async function () {
          const { tokenFactory3, deployer } = await loadFixture(deployTokenFixture);
          await expect(tokenFactory3.deposit(ethers.constants.MaxUint256, deployer.address)).to.be.revertedWithCustomError(tokenFactory3, 'TokenFactory__DepositMoreThanMax')
        })
