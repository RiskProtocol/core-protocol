...
import { BASE_TOKEN_ADDRESS, BASE_TOKEN_DECIMALS } from '../helper-hardhat-config';
...
const func: DeployFunction = async ({ getNamedAccounts, deployments, network }) => {
    ...
    */
    ...
    const TokenFactory = await deploy("TokenFactory", {
        from: deployer,
        args: [BASE_TOKEN_ADDRESS, BASE_TOKEN_DECIMALS, ethUsdPriceFeedAddress],
        log: true,
    ...
    if (!developmentChains.includes(network.name) && process.env.ETHERSCAN_API_KEY) {
        await verify(TokenFactory.address, [BASE_TOKEN_ADDRESS, BASE_TOKEN_DECIMALS, ethUsdPriceFeedAddress])
    }
};
export default func;
func.tags = ["all"]; 