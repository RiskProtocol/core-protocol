...
import { developmentChains, networkConfig, BASE_TOKEN_ADDRESS, REBASE_INTERVAL, CHAINLINK_TOKEN_ADDRESS, CHAINLINK_ORACLE_ADDRESS, CHAINLINK_JOB_ID, LINK_FEE, CURRENT_TIMESTAMP} from '../helper-hardhat-config';
...
const func: DeployFunction = async ({ getNamedAccounts, deployments, network }) => {
    ...
    */
    ...
    const TokenFactory = await deploy("TokenFactory", {
        from: deployer,
        args: [baseTokenAddress, priceFeedAddress, REBASE_INTERVAL, CHAINLINK_TOKEN_ADDRESS, CHAINLINK_ORACLE_ADDRESS, CHAINLINK_JOB_ID, LINK_FEE, CURRENT_TIMESTAMP],
        log: true,
    ...
    if (!developmentChains.includes(network.name) && process.env.ETHERSCAN_API_KEY || developmentChains.includes('sepolia')) {
        await verify(TokenFactory.address, [baseTokenAddress, priceFeedAddress, REBASE_INTERVAL, CHAINLINK_TOKEN_ADDRESS, CHAINLINK_ORACLE_ADDRESS, CHAINLINK_JOB_ID, LINK_FEE, CURRENT_TIMESTAMP])
    }     
};
export default func;
func.tags = ["all"]; 