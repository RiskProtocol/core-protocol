...
const func: DeployFunction = async ({ getNamedAccounts, deployments, network }) => {
    ...
    const DevToken1 = await deploy("DevToken", {
        from: deployer,
        args: [TOKEN1_NAME, TOKEN1_SYMBOL, tokenFactory.address],
        log: true,
    ...
    if (!developmentChains.includes(network.name) && process.env.ETHERSCAN_API_KEY) {
        await verify(DevToken1.address, [TOKEN1_NAME, TOKEN1_SYMBOL, tokenFactory.address])
    }  
    ...
    const DevToken2 = await deploy("DevToken", {
        from: deployer,
        args: [TOKEN2_NAME, TOKEN2_SYMBOL, tokenFactory.address],
        log: true,
        // we need to wait if on a live network so we can verify properly
        waitConfirmations: networkConfig[network.name].blockConfirmations || 1,
    })
    ...
    if (!developmentChains.includes(network.name) && process.env.ETHERSCAN_API_KEY) {
        await verify(DevToken2.address, [TOKEN2_NAME, TOKEN2_SYMBOL, tokenFactory.address])
    }  
    log("----------------------------------")   
