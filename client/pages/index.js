...
import {tokenFactoryAddress, devTokenXAddress, devTokenYAddress, tokenFactoryAbi, devTokenAbi, underlyingTokenAddress,uniswapV2RouterAddress, uniswapV2RouterABI, uniswapV2FactoryAddress, uniswapV2FactoryABI, uniswapV2PairABI} from './../contants'
...
function App() {
  ...
  async function tokenSupply(devToken = 'x') {
    if (typeof window.ethereum !== 'undefined') {
      ...
      try {
        ...
        console.log(`total supply of ${ devToken=='x'? 'tokenX':'tokenY'} is ${data.toString()}`)
      } catch (err) {
        console.log("Error: ", err)
      }
    }    
  ...
  async function createPair() {
    if (typeof window.ethereum !== 'undefined') {   
      const provider = new ethers.providers.Web3Provider(window.ethereum)   
      const signer = provider.getSigner();
      const contract = new ethers.Contract(uniswapV2FactoryAddress, uniswapV2FactoryABI, signer)     
      try {
        const data = await contract.createPair(devTokenXAddress, underlyingTokenAddress)
        const receipt = await data.wait()
        console.log(`Response is  ${receipt.blockNumber}`)
      } catch (err) {
        console.log("Error: ", err)
      }
    }    
  ...
  async function getPair() {
    if (typeof window.ethereum !== 'undefined') {   
      ...
      try {
        const data = await contract.getPair(devTokenXAddress, underlyingTokenAddress)
  ...
  async function addLiquidity() {    
    if (typeof window.ethereum !== 'undefined') {  
      ...
      const devTokenX = new ethers.Contract(devTokenXAddress, devTokenAbi, signer)  
      const devTokenY = new ethers.Contract(underlyingTokenAddress, devTokenAbi, signer)  

      try {
        const allowance1 = await devTokenX.allowance('0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266', uniswapV2RouterAddress)
        const allowance2 = await devTokenY.allowance('0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266', uniswapV2RouterAddress)    
        ...
        const data = await contract.addLiquidity(devTokenXAddress, underlyingTokenAddress, ethAmount, ethAmount, 0, 0, '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266',  Math.floor(Date.now() / 1000) + 300)
        const receipt = await data.wait()
        console.log(`Response is  ${receipt.blockNumber}`)
      } catch (err) {
        console.log("Error: ", err)
  ...
  async function checkTokenPairBalance() {   
    if (typeof window.ethereum !== 'undefined') {   
      const provider = new ethers.providers.Web3Provider(window.ethereum)   
      const signer = provider.getSigner();
      const contract = new ethers.Contract(tokenPairAddress, uniswapV2PairABI, provider)     
      try {
  ...
  async function performTrade() {   
    if (typeof window.ethereum !== 'undefined') {   
      const provider = new ethers.providers.Web3Provider(window.ethereum)   
      const signer = provider.getSigner();
      const uniswapRouterContract = new ethers.Contract(uniswapV2RouterAddress, uniswapV2RouterABI, signer) 
      try {
        ...
        const options = { gasPrice: ethers.utils.parseUnits("100", "gwei"), gasLimit: 200000 };

        const devTokenX = new ethers.Contract(devTokenXAddress, devTokenAbi, signer)
        await devTokenX.approve(uniswapV2RouterAddress,amountIn) 

        const tx = await uniswapRouterContract.swapExactTokensForTokens(amountIn, amountOutMin, path, '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266', deadline, options);
        console.log("Transaction sent:", tx.hash);
        console.log(`Trading result is:  ${tx}`)
      } catch (err) {
        console.log("Error: ", err)
      }
    }    
  ...
  async function callSync() {   
    if (typeof window.ethereum !== 'undefined') {   
      const provider = new ethers.providers.Web3Provider(window.ethereum)   
      const signer = provider.getSigner();
      const contract = new ethers.Contract(tokenPairAddress, uniswapV2PairABI, signer)     
      try {
  ...
  async function estimatedPrice() {   
    if (typeof window.ethereum !== 'undefined') { 
      const amountIn = ethers.utils.parseUnits('10', '18'); 
      const provider = new ethers.providers.Web3Provider(window.ethereum)   
      const signer = provider.getSigner();
      const contract = new ethers.Contract(uniswapV2RouterAddress, uniswapV2RouterABI, provider)     
      try {
        const data = await contract.getAmountOut(amountIn, devTokenXAddress, devTokenYAddress)
        console.log(`estimated amount of Token Y for 10 Token X:  ${data}`)
      } catch (err) {
        console.log("Error: ", err)
      }
    }    
  ...
  return (
    <div className="App">
      ...
      <button style={buttonStyle} onClick={createPair}>Create Trading Pair</button>    
      <button style={buttonStyle} onClick={getPair}>Get Trading Pair Address</button>
      <button style={buttonStyle} onClick={addLiquidity}>Add Liquidity</button>  
      <button style={buttonStyle} onClick={checkTokenPairBalance}>Check Trading pair reserve</button>
      <button style={buttonStyle} onClick={checkUserTokenPairBalance}>Check User Balance in Trading Pair</button>       
      <button style={buttonStyle} onClick={performTrade}>Perform a Trade</button>  
      <button style={buttonStyle} onClick={callSync}>Trigger the Sync function</button> 
   ...
   <br/><br/><br/>
        ...
        <button style={buttonStyle} onClick={tokenSupply.bind(this, 'x')}>Total Supply of Token X</button>
        <button style={buttonStyle} onClick={tokenSupply.bind(this, 'y')}>Total Supply of Token Y</button>
      </div>
    </div>
  );
}
